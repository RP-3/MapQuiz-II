...
class ChallengeSets {
    ...
    static let EN_COUNTIES = ChallengeSet("EN_COUNTIES", "English Counties (Ceremonial)",  img("pickUsStates"), img("usStates"), "ceremonial counties",  region(53, -2, 10),    48*6,  ["englandCounties"])
    static let SC_COUNTIES = ChallengeSet("SC_COUNTIES", "Scottish Counties (Ceremonial)",  img("pickUsStates"), img("usStates"), "ceremonial counties", region(56, -4, 10),    35*6,  ["scotlandCounties"])
    static let WA_COUNTIES = ChallengeSet("WA_COUNTIES", "Welsh Counties (Ceremonial)",  img("pickUsStates"), img("usStates"), "ceremonial counties",    region(52, -3.5, 5),   8*6,   ["walesCounties"])
    static let GBR_COUNTIES = ChallengeSet("GBR_COUNTIES", "All British Counties (Ceremonial)",  img("pickUsStates"), img("usStates"), "ceremonial counties", region(55, -3, 12), 91*6,  ["englandCounties", "scotlandCounties", "walesCounties"])

    static let all = [
        ChallengeSetCollection(header: "Countries of the World", challengeSets: [NA, SA, AS, EU, OC, AF, WORLD]),
        ChallengeSetCollection(header: "United Kingdom", challengeSets: [EN_COUNTIES, SC_COUNTIES, WA_COUNTIES, GBR_COUNTIES]),
